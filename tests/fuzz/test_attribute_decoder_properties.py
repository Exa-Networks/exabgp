...
it accepts, it must be able to render.  Nothing held the *attribute* decoders to the same
rule, and that is the gap issue #1426 fell through.  A `traffic-rate` extended community
carrying a NaN decoded happily and then raised `ValueError` out of
`'rate-limit:%d' % self.rate`, inside the API writer, outside the decode boundary, where no
NOTIFICATION can be sent.

The rule is the same one, stated for attributes: **an attribute section which decodes must
render.**  Whether the bytes are accepted is the decoder's business, and `Attributes.parse`
deliberately treats `IndexError` and `ValueError` out of a decoder as the RFC 7606 discard
and treat-as-withdraw paths.  What happens *after* a successful decode is not negotiable.

The rendering is driven through `Attributes`, not through the attribute object, on purpose.
The collection decides per attribute code whether the JSON comes from `json()` or from
`str()`, so an attribute which raises `NotImplementedError` called directly may be
perfectly renderable in the only place it is ever rendered.  Asking the object is a test of
the test; asking the collection is the path `Processes.message()` takes, which is where
#1426 died.

Note what a sweep over `st.binary()` alone would have done here, because it is the trap the
fuzz README warns about.  An extended community is looked up by its first two bytes, so
random bytes land on a registered subtype roughly once in sixty five thousand draws: two
hundred examples per code would have swept the traffic-rate decoder zero times and reported
a clean run.  The community strategies below therefore *build* a header for each registered
(type, subtype) and fuzz only the value, which is what puts the decoder under test rather
than the dispatch in front of it.
"""
...
from exabgp.bgp.message.update.attribute.community.extended import ExtendedCommunity, ExtendedCommunityIPv6
from exabgp.environment import getenv
from exabgp.logger import log

pytestmark = pytest.mark.fuzz

# Attributes.parse logs what it was handed, so the logger has to exist to call it at all.
log.init(getenv())
...
def payload(min_size_bytes, max_size_bytes):
...
def decodes(flag, aid, value):
    ...
    RFC 7606 discard and treat-as-withdraw paths `Attributes.parse` catches on purpose.
    All three are the decoder doing its job.
    """
    try:
        return Attributes().parse(section(flag, aid, value), Direction.IN, negotiated())
    except (Notify, IndexError, ValueError):
        return None


def renders(attributes, what):
...
def test_a_decoded_attribute_can_be_rendered(registered, value) -> None:
...
def test_a_decoded_extended_community_can_be_rendered(registered, high_nibble, value) -> None:
...
def test_a_decoded_ipv6_extended_community_can_be_rendered(registered, high_nibble, value) -> None:
