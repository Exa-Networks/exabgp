...
### Registered NLRI Decoders (`test_nlri_decoder_properties.py`)

Runs over **every** family in `NLRI.registered_nlri` rather than a hand picked
list, so a newly registered family is covered the day it is registered:

- Arbitrary bytes decode or raise Notify, never a raw Python exception
- A length prefix which does not match the payload is not trusted
- EVPN route types, BGP-LS TLV headers and MUP route types are generated with a
  plausible header so the generated bytes reach past the first length check
- What a decoder accepts, it re-encodes into something it accepts again, with
  the same index (idempotence)

A decoded NLRI also has to survive `json()`, `str()`, `index()` and `hash()`:
several decoders used to accept malformed routes and fail later, in the API
output, where the failure is much harder to trace back to the peer which sent
it. The decoders which report a route they could not parse with `NLRI.INVALID`,
Flow among them, count as having rejected it since every caller drops it.

The falsifying examples this suite produced are kept as plain regression tests
in `tests/unit/test_nlri_wire_bounds.py`.
