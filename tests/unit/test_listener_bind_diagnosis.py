...
listen_on() used to pick its message from the effective uid and the port number
alone, so any failure on a privileged port as a normal user was reported as
"you may need to run ExaBGP as root" and the real reason was dropped.  An MD5
key the kernel cannot install fails the same call and needs a different answer.
"""
...
def _critical(log: MagicMock) -> list:
...
def test_every_bind_failure_reports_its_own_reason(failure: Any, expected: str) -> None:
    listener = Listener(MagicMock())
