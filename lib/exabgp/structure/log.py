...
#			self._parser = False
	...
	def network (self,message,recorder='info'):
		if self._network and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def wire (self,message,recorder='debug'):
		if self._wire and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def message (self,message,recorder='info'):
		if self._message and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def configuration (self,message,recorder='info'):
		if self._configuration and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def supervisor (self,message,recorder='info'):
		if self._supervisor and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def rib (self,message,recorder='info'):
		if self._rib and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def timers (self,message,recorder='debug'):
		if self._timer and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def daemon (self,message,recorder='info'):
		if self._daemon and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def processes (self,message,recorder='info'):
		if self._processes and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
	...
	def routes (self,message,recorder='info'):
		if self._routes and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
			getattr(self,recorder)(message,'routes')
		else:
			self._record(time.localtime(),'routes',recorder.upper(),message)

	# show how the message received are parsed
	def parser (self,message,recorder='info'):
		if self._parser and getattr(syslog,'LOG_%s' % recorder.upper()) <= self.level:
