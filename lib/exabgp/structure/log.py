...
#			self._parser = False
	...
	def network (self,message,recorder='info'):
		...
		if self._network and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def wire (self,message,recorder='debug'):
		recorder = short(recorder)
		if self._wire and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def message (self,message,recorder='info'):
		recorder = short(recorder)
		if self._message and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def configuration (self,message,recorder='info'):
		recorder = short(recorder)
		if self._configuration and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def supervisor (self,message,recorder='info'):
		recorder = short(recorder)
		if self._supervisor and getattr(syslog,'LOG_%s' % recorder) <= self.level:
			getattr(self,recorder)(message,'supervisor')
		else:
			self._record(time.localtime(),'supervisor',recorder,message)

	# show the change of rib table
	def rib (self,message,recorder='info'):
		recorder = short(recorder)
		if self._rib and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def timers (self,message,recorder='debug'):
		recorder = short(recorder)
		if self._timer and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def daemon (self,message,recorder='info'):
		recorder = short(recorder)
		if self._daemon and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def processes (self,message,recorder='info'):
		recorder = short(recorder)
		if self._processes and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def routes (self,message,recorder='info'):
		recorder = short(recorder)
		if self._routes and getattr(syslog,'LOG_%s' % recorder) <= self.level:
	...
	def parser (self,message,recorder='info'):
		recorder = short(recorder)
		if self._parser and getattr(syslog,'LOG_%s' % recorder) <= self.level:
