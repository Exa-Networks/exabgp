...
#	'  hold-time 180;\n' \
	...
	def _single_line (self,scope,name,tokens,valid):
		...
		if name in ('neighbor','group'):
			...
			if command == 'merge-parsed-and-raw': return self._set_process_command(scope,'merge-parsed-and-raw',tokens[1:])
	...
	def _multi_process (self,scope,tokens):
		while True:
			r = self._dispatch(scope,'process',[],['run','encoder','receive-parsed','receive-packets','merge-parsed-and-raw','send-packets','receive-routes','receive-updates','receive-refresh','receive-operational','neighbor-changes',  'peer-updates','parse-routes', 'receive-keepalives', 'receive-opens','receive-notifications'])
	...
	def _make_neighbor (self,scope):
		...
		for local_scope in (scope[0],scope[-1]):
			...
			neighbor.api.merge_parsed_and_raw = local_scope.get('merge-parsed-and-raw',False)
