...
MVPN_SHAREDJOIN_IPV4_LENGTH = 22  # 8 (RD) + 4 (Source AS) + 1 (source len) + 4 (IPv4) + 1 (group len) + 4 (IPv4)
MVPN_SHAREDJOIN_IPV6_LENGTH = 46  # 8 (RD) + 4 (Source AS) + 1 (source len) + 16 (IPv6) + 1 (group len) + 16 (IPv6)
...
class SharedJoin(MVPN):
    ...
    def unpack(cls, data, afi):
        ...
        check_source_and_group(data, 12, 'C-Multicast Shared Tree Join route')
        cursor = 0
