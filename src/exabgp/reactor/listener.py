...
class Listener:
    ...
    def listen_on(self, local_addr, remote_addr, port, md5_password, md5_base64, ttl_in):
        try:
            ...
            log.debug(
                lambda local_addr=local_addr,
                port=port,
                md5_suffix=md5_suffix: f'listening for BGP session(s) on {local_addr}:{port}{md5_suffix}',
                'network',
            )
            return True
        except NetworkError as exc:
            if os.geteuid() != 0 and port <= MAX_PRIVILEGED_PORT:
                log.critical(
                    lambda local_addr=local_addr,
                    port=port: f'can not bind to {local_addr}:{port}, you may need to run ExaBGP as root',
                    'network',
                )
            else:
