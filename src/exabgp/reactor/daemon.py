...
class Daemon(object):
    ...
    def savepid(self):
        ...
        try:
            ...
            try:
                ...
                if self.check_pid(int(pid)):
                    logfunc.debug(lambda: 'PIDfile already exists and program still running %s' % self.pid, 'daemon')
                    return False
                else:
                    # If pid is not running, reopen file without O_EXCL
                    fd = os.open(self.pid, flags ^ os.O_EXCL, mode)
            except (OSError, IOError, ValueError):
                logfunc.debug(lambda: 'issue accessing PID file %s (most likely permission or ownership)' % self.pid, 'daemon')
                return False
        ...
        try:
            ...
            logfunc.warning(lambda: 'Can not create PIDfile %s' % self.pid, 'daemon')
            return False
        logfunc.warning(lambda: 'Created PIDfile %s with value %d' % (self.pid, ownid), 'daemon')
        return True
    ...
    def removepid(self):
        ...
        try:
            ...
            if exc.errno == errno.ENOENT:
                ...
                logfunc.error(lambda: 'Can not remove PIDfile %s' % self.pid, 'daemon')
                return
        logfunc.debug(lambda: 'Removed PIDfile %s' % self.pid, 'daemon')
    ...
    def daemonise(self):
        ...
        if logging.enable and logging.destination.lower() in ('stdout', 'stderr'):
            logfunc.critical(lambda: 'ExaBGP can not fork when logs are going to %s' % log.destination.lower(), 'daemon')
            return
        ...
        def fork_exit():
            try:
                ...
                logfunc.critical(lambda: 'can not fork, errno %d : %s' % (exc.errno, exc.strerror), 'daemon')
