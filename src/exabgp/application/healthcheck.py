...
def neighbor_address(value: str) -> IPv4Address | IPv6Address | str:
...
def attribute_value(value: str) -> str:
    ...
    for character in value:
        if character == '\t' or (character.isprintable() and character != '\x7f'):
            continue
        raise argparse.ArgumentTypeError(f'{value!r} contains the control character {character!r}')
    return value
...
def system_ips(
    ...
    if sys.platform.startswith('linux'):
        ...
        for ifname in ifnames:
            cmd = subprocess.Popen(['ip', '-o', 'address', 'show', 'dev', ifname], shell=False, stdout=subprocess.PIPE)
...
def loop(options: argparse.Namespace) -> None:
    ...
    def exabgp(target: States) -> None:
        ...
        if options.neighbors and not any(n == '*' for n in options.neighbors):
            prefix = ', '.join(f'peer {neighbor}' for neighbor in options.neighbors)
        else:
            prefix = 'peer *'
