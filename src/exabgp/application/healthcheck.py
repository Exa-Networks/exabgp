...
def system_ips(ip_ifnames, label, label_only, label_exact_match):
    ...
    if sys.platform.startswith('linux'):
        ...
        labelre = re.compile(r'.*\s+(?:' + '|'.join(re.escape(str(ifn)) for ifn in ifnames) + r'):(?P<label>[^\\\s]+).*')
        for ifname in ifnames:
            # Build command as list to avoid injection (even though ifname is validated)
            cmd = subprocess.Popen(['/sbin/ip', '-o', 'address', 'show', 'dev', str(ifname)],
        ...
        for ifname in ifnames:
            # Build command as list to avoid injection (even though ifname is validated)
            cmd = subprocess.Popen(['/sbin/ifconfig', str(ifname)],
                                   shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            output += [ line for line in cmd.stdout]
